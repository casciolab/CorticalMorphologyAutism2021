function [ a, cb ] = SurfStatViewData2( data, surf, title, background );
...
vc = [load('lh.curv.mean.txt'); ...
      load('rh.curv.mean.txt')]';
vc(vc > 0)=1;
vc(vc < -0)=-1;
vc = -vc;
vc = vc - min(vc);
...
cmap=gray(256)*0.3+0.4;
intv = size(cmap,1);
data2 = vc;
[~,~,id] = histcounts(data2, intv);
...
intv = size(cmap,1);
data2 = data;
data2(data < clim(1)) = clim(1);
data2(data > clim(2)) = clim(2);
[~,~,id] = histcounts(data2, intv);
...
cmap_lh(data(vl) < 6.4,:) = cmap_lh2(data(vl) < 6.4,:);
cmap_rh(data(vr) < 6.4,:) = cmap_rh2(data(vr) < 6.4,:);
cmap_hemi(data < 6.4,:) = cmap_hemi2(data < 6.4,:);

a(1)=axes('position',[0.055 0.62 h*3/4 w]);
trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),...
    'FaceVertexCData', cmap_lh,'EdgeColor','none');
view(-90,0); 
...
trisurf(surf.tri,surf.coord(1,:),surf.coord(2,:),surf.coord(3,:),...
    'FaceVertexCData', cmap_hemi,'EdgeColor','none');
view(0,90); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
lighting phong; material dull; shading interp;
...
if cut<t
    ...
    trisurf(surf.tri(tr,:)-cuv,surf.coord(1,vr),surf.coord(2,vr),surf.coord(3,vr),...
        'FaceVertexCData', cmap_rh,'EdgeColor','none');
    view(90,0);
    daspect([1 1 1]); axis tight; camlight; axis vis3d off;
    lighting phong; material dull; shading interp;
else
