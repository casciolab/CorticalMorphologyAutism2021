function [ a, cb ] = SurfStatView2( struct, surf, title, background);
...
if ~isstruct(struct)
    ...
    if isfield(struct,'P')
        ...
        struct.thresh=struct.thresh/2;

        a=SurfStatViewData2(tt,surf,title,background);
        cb = SurfStatColLim( [0 255]*struct.thresh*1.01 );
        h=get(cb,'Title');
        set(h,'String',title,'FontSize',12);

        set(cb,'XLim',[0 255]*struct.thresh);
        h=get(cb,'Children');
        set(h,'XData',[0 255]*struct.thresh);
        set(cb,'XTick',(2+(0:2)/2*253)*struct.thresh);
        set(cb,'XTickLabel',num2str(struct.thresh*2*(2:-1:0)'/2),'FontSize',10);
        xl=get(cb,'XLabel');
        set(xl,'String','','FontSize',11);
